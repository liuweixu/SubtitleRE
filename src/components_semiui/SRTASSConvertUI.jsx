...
const SRTASSConvertUI = () => {
  const { inputText, click, onClear } = SRTASSConvert();
  //获取输入框的目录地址
  ...
  return (
    <Form labelCol={{ span: 10 }} labelPosition="inset" labelAlign="right">
      <Form.Input
        ...
        label="SRT输入目录"
        trigger="blur"
        placeholder="请输入SRT字幕所在目录"
        onChange={handleInputChange}
      />
      <FormChange field_name={"srtInputDir"} setInput={setInput} />
      <Form.Input
        id="suffix"
        field="srtSuffix"
      ...
      />
      <FormChange field_name={"assOuputDir"} setInput={setOutput} />
      <Form.Input
        id="style"
