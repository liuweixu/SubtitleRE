...
function mkv_extracting(data, language){
    ...
    for(let i = 0; i < lines.length; i++){
        ...
        if(isEvent){
            ...
            const is_japanese = style_name.toUpperCase().includes('JP') || style_name.toUpperCase().includes('JA') || 
                                style_name.includes('少女日常日文') || style_name.includes('日文歌詞方案') || style_name.includes('正 文 日')
            if((language.toLowerCase() === 'chs' && is_japanese) || (language.toLowerCase() === 'jp' && !is_japanese)){
                continue
            }
        }
        new_lines.push(line)
...
function MKVExtractor(input, file, track, output, language, basename, callback){
    const mkvInputFile = path.join(input, file)
    const mkvTempFile = path.join(output, basename + "temp.sub")
    
    exec(`mkvextract tracks "${mkvInputFile}" ${track}:"${mkvTempFile}"`, (error) => {
        if(error){
            console.log('读取错误')
        ...
        fs.readFile(mkvTempFile, 'utf-8', (error, data) => {
            ...
            if(detect_format == 'ass'){
                ...
                const mkvOutputFile = path.join(output, basename + '.' + language.toLowerCase() + '.ass')
